...
export function StatsCards({ activities, allActivities, year, filter, onSelectActivity }: StatsCardsProps) {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 sm:auto-rows-fr">
      {/* Yearly Goal */}
      <div className="bg-[var(--color-card)] border border-[var(--color-border)] rounded-xl p-5 h-full lg:h-[248px] flex flex-col justify-center transition-all duration-300 hover:shadow-lg hover:shadow-[var(--color-accent)]/5 hover:border-[var(--color-accent)]/30 hover:bg-[var(--color-accent)]/5">
        <p className="min-h-8 text-xs uppercase tracking-wider leading-4 text-[var(--color-muted)] mb-2 flex items-start gap-1.5">
